...
class BaseRunner:
    ...
    def run(self):
        if self.cfg.inference_only:
            ...
            self.train()
            print("************************************Test F1:")
            return (1 - self.metric["related_test_f1"])
    ...
    def train(self):
        ...
        for global_step in range(self.cfg.max_steps):
            memory, norm_term = self.trainer.train_one_step(memory, norm_term)

            if (global_step+1)%self.cfg.logging_steps == 0:
                self.trainer.write_log()

            if global_step > 15000 and (global_step+1)%self.cfg.eval_steps==0:
...
class Runner(BaseRunner):
    ...
    def eval_and_update(self, global_step):
        ...
        if test_f1> self.metric["best_dev_f1"]:
            self.metric["best_dev_f1"] = test_f1
            self.metric["related_test_f1"] = dev_f1

            self.report_result(test_c, dev_c, global_step)
