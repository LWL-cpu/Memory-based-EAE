...
class BartAttention(nn.Module):
    ...
    def forward(
        ...
        query_states_ = self._shape(query_states, tgt_len, bsz).view(*proj_shape)
        key_states_ = key_states.view(*proj_shape)
        value_states = value_states.view(*proj_shape)

        src_len = key_states_.size(1)

        attn_weights = torch.bmm(query_states_, key_states_.transpose(1, 2)).view(bsz, self.num_heads, tgt_len,
                                                                                      src_len)
    ...
    def _update_memory(self, key_states, value_states, memory, norm_term):
        ...
        if memory is not None:
            memory = memory + torch.matmul(key_states.transpose(-2, -1), value_states) * self.scaling
        else:
...
class BartCrossAttention(nn.Module):
    ...
    def __init__(
        ...
        self.gate1 = nn.Linear(embed_dim // self.num_heads, 1)
        self.gate2 = nn.Linear(embed_dim // self.num_heads, 1)
    ...
    def forward(
        ...
        proj_shape = (bsz, self.num_heads, -1, self.head_dim)
        proj_shape_ = (bsz* self.num_heads, -1, self.head_dim)
        query_states_ = self._shape(query_states, tgt_len, bsz).view(*proj_shape)
        key_states_ = key_states.view(*proj_shape)
        value_states = value_states.view(*proj_shape)
        ...
        src_len = key_states_.size(2)

        attn_weights = torch.bmm(query_states_.view(*proj_shape_), key_states_.view(*proj_shape_).transpose(1, 2)).view(bsz, self.num_heads, tgt_len,
                                                                                      src_len)
        attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len)
        attn_weights = attn_weights * self.scaling
        if attn_weights.size() != (bsz * self.num_heads, tgt_len, src_len):
        ...
        attn_output = torch.bmm(attn_probs, value_states.view(*proj_shape_))
        ...
        if memory_output is None:
            ...
            memory_output = memory_output.view(bsz, self.num_heads, tgt_len, self.head_dim)
            combined_output = (
                F.sigmoid(self.gate1(memory_output) + self.gate2(attn_output)) * memory_output
                + (1 - F.sigmoid(self.gate1(memory_output) + self.gate2(attn_output))) * attn_output
            )
            # print(F.sigmoid(self.gate1(memory_output) + self.gate2(attn_output)).max())
            # print(F.sigmoid(self.gate1(memory_output) + self.gate2(attn_output)).min())


        # combined_output = combined_output.view(bsz, self.num_heads, tgt_len, self.head_dim)
        combined_output = combined_output.transpose(1, 2)
        combined_output = combined_output.reshape(bsz, tgt_len, embed_dim)
        ...
        return combined_output, attn_weights_reshaped, past_key_value, memory, norm_term,
    def _retrieve_from_memory(self, query_states, memory, norm_term):
        # query_states: [batch_size, num_heads, seq_len, head_dim]
        ...
        # Apply ELU activation
        query_states = F.elu(query_states) + 1  # ELU activation + 1 for stability
        memory = memory.unsqueeze(0).repeat(query_states.shape[0], 1, 1, 1)
        norm_term = norm_term.unsqueeze(0).repeat(query_states.shape[0], 1, 1, 1)
        memory_output = torch.matmul(query_states, memory)
    ...
    def _update_memory(self, key_states, value_states, memory, norm_term):
        ...
        if memory is not None:
            memory = memory + torch.matmul(key_states.transpose(-2, -1), value_states).sum(0)
        else:
            memory = torch.matmul(key_states.transpose(-2, -1), value_states).sum(0)

        if norm_term is not None:
            norm_term = norm_term + key_states.sum(
                dim=2, keepdim=True
            ).sum(0)  # Update normalization term
        else:
            norm_term = key_states.sum(
                dim=2, keepdim=True
            ).sum(0)  # Initialize normalization term
...
class BartDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            hidden_states, cross_attn_weights, cross_attn_present_key_value, memory,  norm_term,  = self.encoder_attn(
                hidden_states=hidden_states,
    ...
    def forward(
        ...
        Args:
            ...
            output_hidden_states (:obj:`bool`, `optional`):
                Whether or not to return the hidden states of all layers. See ``hidden_states`` under returned tensors
                for more detail.
            memory (:obj:`bool`, `optional`):
                Whether or not to return the hidden states of all layers. See ``hidden_states`` under returned tensors
                for more detail.
            norm_term (:obj:`bool`, `optional`):
                Whether or not to return the hidden states of all layers. See ``hidden_states`` under returned tensors
                for more detail.
            no_memory_update (:obj:`bool`, `optional`):
                Whether or not to return the hidden states of all layers. See ``hidden_states`` under returned tensors
                for more detail.
            return_dict (:obj:`bool`, `optional`):
                Whether or not to return a :class:`~transformers.file_utils.ModelOutput` instead of a plain tuple.
...
class BartModel(BartPretrainedModel):
    ...
    def forward(
        ...
        output_attentions=None,
        output_hidden_states=None,
        memory=None, # memory项，可以从外部添加
        norm_term=None, # norm项，可以从外部添加
        no_memory_update=False, # 是否更新memory
        return_dict=None,
    ):
        ...
        decoder_outputs = self.decoder(
            ...
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            memory=memory,  # 增加memory
            norm_term=norm_term, # 增加norm_term
            no_memory_update=no_memory_update, # 增加 no_memory_update
            return_dict=return_dict,
        )
        ...
        return Seq2SeqModelOutput(
            ...
            memory=decoder_outputs.memory,
            norm_term=decoder_outputs.norm_term
        )
