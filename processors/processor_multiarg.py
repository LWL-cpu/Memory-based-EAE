...
class ArgumentExtractionDataset(Dataset):
    ...
    def collate_fn(batch):
        ...
        event_trigger = [f.event_trigger[:2] for f in batch]
        ...
        return enc_input_ids, enc_mask_ids,  all_ids, all_mask_ids, \
                ...
                start_position_ids, end_position_ids, event_trigger
        ...
        for example_idx, example in enumerate(examples):
            ...
            for tok in sent:
                if tok not in EXTERNAL_TOKENS:
                    ...
                    trigger_start_char = curr
                    trigger_end_char = curr+len(tok)-1
                    new_tok_t_s = enc.char_to_token(trigger_start_char)
                    new_tok_t_e = enc.char_to_token(trigger_end_char) + 1
                    event_trigger[1] = [new_tok_t_s, new_tok_t_e]
            ...
            if self.prompt_query:
                ...
                if dec_prompt_text:
                    # dec_prompt_text = '<t>' + ' ' + event_trigger[0] + ' ' + '</t>' + ' ' +  dec_prompt_text
            ...
            # all_ids.append(self.tokenizer.convert_tokens_to_ids('</s>'))
            all_ids.extend(dec_prompt_ids)

            # all_mask_ids.append(1)
            all_mask_ids.extend(dec_prompt_mask_ids)
            ...
            for arg in arg_list:
                ...
                if self.prompt_query:
                    ...
                    for matching_result in re.finditer(r'\b'+re.escape(arg)+r'\b', dec_prompt_text.split('.')[0]): 
                        ...
                        prompt_slots["tok_s_off"].append(tok_prompt_s + offset_prompt);prompt_slots["tok_e_off"].append(tok_prompt_e + offset_prompt)
