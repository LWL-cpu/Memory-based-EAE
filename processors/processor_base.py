...
class DSET_processor:
    ...
    def _create_example_rams(self, lines):
        ...
        return examples
    def _create_example(self, lines, over_sample=None, max_num_event=100):
        W = 250
        examples = []
        for line in lines:
            doc_id = line["id"]
            ...
            if self.args.single:
                for event in events:
                    ...
                    examples.append(Event(doc_id, event_idx, text, event_type, event_trigger, event_args, full_text))
                continue
            ...
            for event in events:
                ...
                examples.append(Event(doc_id, 0, context, event_type, event_type_2_events))

            if over_sample == 'double' and num_events > 1:
                examples.append(Event(doc_id, context, event_type_2_events))
                ...
                for i, events_ in enumerate(power_set):
                    ...
                    examples.append(Event('%d-%s' % (i, doc_id), context, event_type_2_events))
    ...
    def generate_dataloader(self, set_type):
        ...
        if self.collate_fn:
            if set_type != 'train':
                dataloader = DataLoader(dataset, sampler=dataset_sampler, batch_size=self.args.infer_batch_size, collate_fn=self.collate_fn)
            else:
